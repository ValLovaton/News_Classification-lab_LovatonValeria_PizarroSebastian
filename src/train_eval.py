...
def train_and_eval(model_id: str, ds_splits: DatasetDict, outdir: str, epochs: float = 2.0,
                   lr: float = 2e-5, bs_train: int = 16, bs_eval: int = 32) -> Dict[str, Any]:
    num_labels = 4
    tokenizer = AutoTokenizer.from_pretrained(model_id, use_fast=True)
    tokenized = ds_splits.map(lambda b: _tokenize_function(b, tokenizer), batched=True)
